...
def GetArgs():
    ...
    parser.add_argument("--vdfs_ip", nargs='+', help="List of IP addresses")
    parser.add_argument("--vdfs_fqdn", nargs='+', help="List of corresponding FQDNs")
...
def getFileServiceDomainConfig(IP_FQDN_DIC):
    networkProfiles = []
...
def main():
    ...
    if not args.vdfs_ip or not args.vdfs_fqdn or len(args.vdfs_ip) != len(args.vdfs_fqdn):
        raise ValueError("IP and FQDN lists must be provided and have the same length.")

    IP_FQDN_DIC = dict(zip(args.vdfs_ip, args.vdfs_fqdn))
    ...
    fsDomainConfig = getFileServiceDomainConfig(IP_FQDN_DIC)
