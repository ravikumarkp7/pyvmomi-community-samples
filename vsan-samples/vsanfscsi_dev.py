...
IP_FQDN_DIC = {"172.16.10.10": "nsxt1.vmware.com",
               "172.16.10.11": "nsxt2.vmware.com",
               "172.16.10.12": "nsxt3.vmware.com"}
SUBNET_MASK = "255.255.0.0"
GATEWAY_ADDRESS = "172.16.10.1"
DNS_SUFFIXES = ["vmware.com"]
...
def main():
   ...
   print(f"OVF download url {ovfUrl}...")
   # ovfUrl = vcfs.FindOvfDownloadUrl(cluster)
   # ovfUrl = "https://build-squid.vcfd.broadcom.net/build/mts/release/bora-24781383/publish/vdfs-fsvm/VMware-vSAN-File-Services-Appliance-9.1.0.0.24781383_OVF10.ovf"
   if not ovfUrl:
      print("Failed to find the OVF download url.")
      return -1
   print(f"Found OVF download url: {ovfUrl}" )

   # Download FSVM OVF files to vCenter
   print(f"Downloading ovf files from {ovfUrl} to vCenter ...")
   ...
   for network_local in cluster.host[0].network:
     if isinstance(network_local, vim.Network) and network_local.name == "testDportGroup":
        net = network_local
        break
