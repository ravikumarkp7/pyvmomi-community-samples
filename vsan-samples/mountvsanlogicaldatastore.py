...
def getLocalVsanDatastore(cluster):
   ...
   localDs = [ds for ds in cluster.datastore if ds.summary.type == 'vsan' \
              and ds.summary.name == 'logicalDsVsanMax']
   print('found the logicalDsVsanMax datastore', localDs)
...
def mountDatastore(si, vsccs, cluster, dsList, vsanConfig, dsConfig, remoteServerClusterConfig):
   spec = vim.vsan.ReconfigSpec(vsanClusterConfig=vsanConfig,
                                datastoreConfig=dsConfig)
   spec.serverClusterConfig = remoteServerClusterConfig
   vsanTask = vsccs.ReconfigureEx(cluster, spec)
   vcTask = vsanapiutils.ConvertVsanTaskToVcTask(vsanTask, si._stub)
   vsanapiutils.WaitForTasks([vcTask], si)
   if vcTask.info.state != 'success':
   ...
   if aboutInfo.apiType == 'VirtualCenter':
      ...
      if len(localDatastore) > 0:
         ...
         if True:
            ...
            remoteServerClusterConfig = vim.vsan.VcRemoteVsanServerClusterConfig()
            affinityObj = vim.vsan.RemoteVsanSiteAffinity()
            affinityObj.serverSite = vim.vsan.RemoteVsanSite(name='Primary')
            siteAffinity = []
            siteAffinity.append(affinityObj)
            clusterUuid = GetClusterUuid(clientCluster)
            remoteServerClusterConfig.serverClusters =[vim.vsan.VcRemoteVsanServerClusterInfo(clusterUuid=GetClusterUuid(serverCluster), networkTopology="Asymmetric", siteAffinity=siteAffinity)]
            remoteServerClusterConfig.serverClusters[0].siteAffinity = siteAffinity
            mountDatastore(si, vsccs, clientCluster, localDatastore, vsanConfig, dsConfig, remoteServerClusterConfig)
      else:
